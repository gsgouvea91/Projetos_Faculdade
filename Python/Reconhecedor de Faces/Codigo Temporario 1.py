...
class Controle:
    ...
    def opcao3(self):
        ...
        #Criar modelo (sample):
        mycmd="opencv_createsamples = -img dado5050.jpg -bg bg.txt -info/info.lst -pngoutput Saida -maxxangle 0.5 -maxyangle -0.5 -maxzangle 0.5 -num 1950" 
        
        #Criar arquivo de vetores das detecções positivas: 
        mycmd1 = "opencv_createsamples -info info/info.lst -num 1950 -w 20 -h 20 -vec positives.vec"
        ...
        os.system(mycmd1)
        os.system(mycmd2)
        #Voila! Depois disso é só realizar a detecção na opção seguinte, com o novo casacade.
    ...
    def opcao4(self):
        ...
        if  len(file_list) != 0:
            for file in file_list:
                ...
                for (classifier) in classifiers:
                    test = format(len(classifies))
                    aux1 = int(test)
                    print("Faces:",test)
                    
                # Coloca os quadrados nas faces
                for (x,y,w,h) in classifiers:
                    cv2.rectangle(img,(x,y),(x+w,y+h),(255,0,0),2)
                    roi_gray = gray[y:y+h, x:x+w]
                    roi_color = img[y:y+h, x:x+w]
...
cascade_treinado = cv2.CascadeClassifier('../haarcascades/<nome do arquivo xml>')
